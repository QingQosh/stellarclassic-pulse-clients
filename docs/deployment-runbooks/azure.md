...
Deploys StellarClassicPulse as a Docker container on a self-managed **Azure VM**
...
the primary Azure path for StellarClassicPulse.
...
```bash
RG=stellarclassic-pulse-rg
LOCATION=eastus

az group create --name "$RG" --location "$LOCATION"

az network vnet create \
  --resource-group "$RG" --name stellarclassic-pulse-vnet \
  --address-prefix 10.1.0.0/16 \
  --subnet-name appgw-subnet --subnet-prefix 10.1.1.0/24

az network vnet subnet create \
  --resource-group "$RG" --vnet-name stellarclassic-pulse-vnet \
...
```bash
az network nsg create --resource-group "$RG" --name stellarclassic-pulse-app-nsg

az network nsg rule create \
  --resource-group "$RG" --nsg-name stellarclassic-pulse-app-nsg \
...
az network vnet subnet update \
  --resource-group "$RG" --vnet-name stellarclassic-pulse-vnet \
  --name app-subnet --network-security-group stellarclassic-pulse-app-nsg
```
...
az postgres flexible-server create \
  --resource-group "$RG" --name stellarclassic-pulse-db \
  --location "$LOCATION" \
  --vnet stellarclassic-pulse-vnet --subnet db-subnet \
...
az postgres flexible-server db create \
  --resource-group "$RG" --server-name stellarclassic-pulse-db \
  --database-name stellarclassic_pulse
```
...
```bash
az acr create --resource-group "$RG" --name stellarclassicpulseacr --sku Basic
az acr login --name stellarclassicpulseacr

docker build -t stellarclassic-pulse .
docker tag stellarclassic-pulse:latest stellarclassicpulseacr.azurecr.io/stellarclassic-pulse:latest
docker push stellarclassicpulseacr.azurecr.io/stellarclassic-pulse:latest
```
...
runcmd:
  ...
  - az acr login --name stellarclassicpulseacr
  - docker pull stellarclassicpulseacr.azurecr.io/stellarclassic-pulse:latest
  - |
    cat > /etc/stellarclassic-pulse.env <<ENV
    DATABASE_URL=postgres://soroban_admin:CHANGE_ME@${DB_HOST}:5432/stellarclassic_pulse
  ...
  - docker run -d --name stellarclassic-pulse --restart unless-stopped
      --env-file /etc/stellarclassic-pulse.env -p 3000:3000
      stellarclassicpulseacr.azurecr.io/stellarclassic-pulse:latest
EOF

az vm create \
  --resource-group "$RG" --name stellarclassic-pulse-app \
  --image Ubuntu2204 \
  --size Standard_B2s \
  --vnet-name stellarclassic-pulse-vnet --subnet app-subnet \
  --nsg stellarclassic-pulse-app-nsg \
  --public-ip-address "" \
  --assign-identity \
...
```bash
VM_IDENTITY=$(az vm show --resource-group "$RG" --name stellarclassic-pulse-app \
  --query identity.principalId -o tsv)
ACR_ID=$(az acr show --name stellarclassicpulseacr --query id -o tsv)
...
az network public-ip create \
  --resource-group "$RG" --name stellarclassic-pulse-pip --sku Standard --allocation-method Static

VM_PRIVATE_IP=$(az vm list-ip-addresses --resource-group "$RG" --name stellarclassic-pulse-app \
  --query "[0].virtualMachine.network.privateIpAddresses[0]" -o tsv)

az network application-gateway create \
  --resource-group "$RG" --name stellarclassic-pulse-appgw \
  --location "$LOCATION" \
  --sku Standard_v2 --capacity 2 \
  --vnet-name stellarclassic-pulse-vnet --subnet appgw-subnet \
  --public-ip-address stellarclassic-pulse-pip \
  --servers "$VM_PRIVATE_IP" \
  --frontend-port 443 \
  --cert-file stellarclassic-pulse.pfx --cert-password "CHANGE_ME" \
  --http-settings-port 3000 --http-settings-protocol Http

az network application-gateway probe create \
  --resource-group "$RG" --gateway-name stellarclassic-pulse-appgw \
  --name stellarclassic-pulse-probe \
  --protocol Http --host-name-from-http-settings true \
  --path /healthz/ready --interval 30 --timeout 5 --threshold 2

az network application-gateway http-settings update \
  --resource-group "$RG" --gateway-name stellarclassic-pulse-appgw \
  --name appGatewayBackendHttpSettings --probe stellarclassic-pulse-probe
```

Point your DNS `A` record for `pulse.example.com` at the Standard public IP:

```bash
az network public-ip show --resource-group "$RG" --name stellarclassic-pulse-pip \
...
- **Bad app deploy**: run the previous image tag on the VM and restart:
  ...
  az vm run-command invoke \
    --resource-group "$RG" --name stellarclassic-pulse-app \
    --command-id RunShellScript \
    --scripts 'docker pull stellarclassicpulseacr.azurecr.io/stellarclassic-pulse:<previous-tag> &&
               docker stop stellarclassic-pulse && docker rm stellarclassic-pulse &&
               docker run -d --name stellarclassic-pulse --restart unless-stopped
               --env-file /etc/stellarclassic-pulse.env -p 3000:3000
               stellarclassicpulseacr.azurecr.io/stellarclassic-pulse:<previous-tag>'
  ```
...
|---|---|---|
| `show-backend-health` reports `Unhealthy` | NSG blocks Application Gateway subnet → app subnet on 3000, or app not listening | Confirm the NSG rule's source is `10.1.1.0/24` (the `appgw-subnet` range); `az vm run-command invoke ... --scripts 'docker logs stellarclassic-pulse --tail 100'` |
| Application Gateway returns `502` | Backend pool points at the wrong/old private IP, or the container crashed | `az network application-gateway show --resource-group "$RG" --name stellarclassic-pulse-appgw --query "backendAddressPools"`; check container logs |
...
| App can't reach the database | Subnet delegation missing on `db-subnet`, or VNet integration not enabled on the Flexible Server | `az postgres flexible-server show --resource-group "$RG" --name stellarclassic-pulse-db --query "network"`; confirm `delegatedSubnetResourceId` is set |
