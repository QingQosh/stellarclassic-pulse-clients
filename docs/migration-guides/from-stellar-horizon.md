...
activity, who want to move that workflow onto StellarClassic Pulse's
...
to the whole ledger. StellarClassic Pulse instead indexes decoded Soroban contract
...
| Horizon concept | StellarClassic Pulse equivalent |
|---|---|
...
| Streaming via Horizon SSE (`Accept: text/event-stream`) | StellarClassic Pulse's own SSE stream — see [event-feeds.md](../event-feeds.md) and [sse-reconnection.md](../sse-reconnection.md) |
...
| Manually decoding XDR event data | StellarClassic Pulse decodes and validates XDR before storage (`record_xdr_validation_pass`/`_fail` in `metrics.rs`) |
...
2. **Recreate each consumer as a StellarClassic Pulse subscription.** Follow
...
- Run both the legacy Horizon-based consumer and the new StellarClassic Pulse
...
- **Ledger range gaps**: if your Horizon consumer's last processed ledger
  and StellarClassic Pulse's indexer start point don't overlap, you'll have a gap.
  Confirm StellarClassic Pulse's `stellarclassic_pulse_indexer_current_ledger` was at or
...
- **Different event identity**: Horizon operation/effect IDs are not the
  same as StellarClassic Pulse's internal event IDs. Reconcile by `(contract_id,
